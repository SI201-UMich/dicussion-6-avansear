...
class PollReader():
    ...
    def likely_voter_polling_average(self):
        ...
        trump_total = 0.0
        ctr = 0
        sample_type = self.data_dict["sample type"]
        harris_list = self.data_dict["Harris result"]
        trump_list = self.data_dict["Trump result"]

        for i in range(len(sample_type)):
            if sample_type[i] == "LV":
                harris_total += harris_list[i]
                trump_total += trump_list[i]
                ctr += 1
        
        return ((harris_total / ctr), (trump_total / ctr))
        pass
    ...
    def polling_history_change(self):
        ...
        total_hl = 0
        for i in harris_late:
            total_hl += i
        hl_avg = total_hl / len(harris_late)

        total_he = 0
        for i in harris_early:
            total_he += i
        he_avg = total_he / len(harris_early)

        total_tl = 0
        for i in trump_late:
            total_tl += i
        tl_avg = total_tl / len(trump_late)

        total_te = 0
        for i in trump_early:
            total_te += i
        te_avg = total_te / len(trump_early)

        return (hl_avg - he_avg, tl_avg - te_avg)
        pass
